...
func GetDbConnection(dbtype string, logLevel logger.LogLevel) *gorm.DB {
	_ = godotenv.Load()
	if dbtype == "mysql" {
	...
	if dbtype == "postgres" {
		var err error
		credentials := fmt.Sprintf("host=%s user=%s password=%s dbname=%s port=5432 sslmode=disable", os.Getenv("POSTGRES_IP"), os.Getenv("POSTGRES_USER"), os.Getenv("POSTGRES_PASSWORD"), os.Getenv("POSTGRES_DATABASE"))
